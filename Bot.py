...
from aiohttp import web  # Render Web Service Port Error ရှင်းရန်
...
async def admin_edit_item(callback: types.CallbackQuery):
    if callback.from_user.id != OWNER_ID: return await callback.answer("ခွင့်ပြုချက်မရှိပါ။")
    item_id = callback.data.split("_")[1]
    prices = load_prices()
    item = next((p for p in prices if p["id"] == item_id), None)
    if not item: return await callback.message.reply("Item ရှာမတွေ့တော့ပါ။")
...
async def user_start_order(callback: types.CallbackQuery, state: FSMContext):
    ...
    prices = load_prices()
    item = next((p for p in prices if p["id"] == item_id), None)
    if not item: return await callback.message.reply("❌ ယခုပစ္စည်းသည် ဝယ်ယူ၍မရတော့ပါ။")
...
async def user_get_screenshot(message: types.Message, state: FSMContext):
    if not message.photo: return await message.reply("❌ ကျေးဇူးပြု၍ Ngwe Lwe Screenshot ဓာတ်ပုံကိုသာ ပို့ပေးပါ။")
    photo_id = message.photo[-1].file_id
    data = await state.get_data()
    item = data.get("buy_item")
    id_zone = data.get("id_zone")

    await message.reply("✅ **သင့် Order ကို လက်ခံရရှိပါပြီ!**\nAdmin မှ စစ်ဆေးပြီး မိနစ်ပိုင်းအတွင်း ဖြည့်သွင်းပေးသွားမည် ဖြစ်ပါသည်။", reply_markup=main_menu_keyboard)
    order_details = f"🔔 **Order အသစ် ရောက်ရှိလာပါပြီ!**\n\n👤 **ဝယ်သူ:** [{message.from_user.first_name}](tg://user?id={message.from_user.id})\n📦 **ပစ္စည်း:** {item['name']}\n💰 **စျေးနှုန်း:** {item['price']} Ks\n🎮 **Game ID (Zone):** `{id_zone}`"
...
async def handle_root(request):
    return web.Response(text="StarGrow Shop Bot is alive!")
...
# --- Main Run ---
async def main():
    await start_web_server()  # Render ရဲ့ Port Binding စစ်တာကို ကျော်ဖြတ်ရန်
    print("StarGrow Shop Bot is starting...")
